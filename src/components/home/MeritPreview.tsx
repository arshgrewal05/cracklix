...
import { Trophy, ShieldCheck, ChevronRight, Medal, Activity, Users } from 'lucide-react';
...
/**
 * @fileOverview Home Punjab Merit Preview v1.0.
 * Displays the top 3-5 scorers across the platform to build social proof.
 */
export default function MeritPreview() {
  ...
  const { data: results, loading } = useCollection<any>(resultsQuery);
           ...
           <div className="space-y-8 md:space-y-12">
              <div className="space-y-6">
                 ...
                 <p className="text-slate-500 font-medium text-base md:text-xl max-w-xl leading-relaxed">
                    Compare your performance with thousands of aspirants across 22 districts. Secure your spot in the official merit list.
                 </p>
              </div>

              <div className="grid grid-cols-2 gap-6">
                 <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 shadow-inner">
                    <p className="text-4xl font-headline font-black text-[#0F172A] tabular-nums">15K+</p>
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">ASPIRANT NODES</p>
                 </div>
                 <div className="p-6 bg-primary/5 rounded-[2rem] border border-primary/10 shadow-inner">
                    <p className="text-4xl font-headline font-black text-primary tabular-nums">22</p>
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">DISTRICTS COVERED</p>
                 </div>
              </div>

              <Button asChild className="h-16 px-10 bg-[#0F172A] hover:bg-black text-white font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-3xl gap-3">
                 ...
                 <CardContent className="p-0">
                    <div className="divide-y divide-slate-50">
                       {loading ? (
                          ...
                          const name = (res.userName && res.userName !== 'Student' && !res.userName.includes('@')) ? res.userName : (res.userEmail?.split('@')[0] || "Aspirant");
                          return (
