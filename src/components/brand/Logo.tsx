 ...
 * FIXED: Removed complex Framer Motion interactions to ensure the link responds on first click.
...
export default function Logo({ className = "", variant = 'light', showTagline = true }: LogoProps) {
  ...
  return (
    <Link href="/" className={`flex items-center gap-1.5 md:gap-3 group pointer-events-auto ${className}`}>
      <div className="relative w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 flex items-center justify-center shrink-0">
