...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-slate-50 bg-[#020617] font-body text-white">
      <div className="mx-auto max-w-7xl px-4 md:px-6 py-6 md:py-14">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-10 items-start text-left">
          ...
          <div className="space-y-4">
            <h3 className="text-[10px] font-black text-primary tracking-widest uppercase">Quick Links</h3>
          ...
          <div className="col-span-1 lg:col-span-1 space-y-4">
            <h3 className="text-[10px] font-black text-primary tracking-widest uppercase">Support</h3>
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-[9px] md:text-sm text-slate-400">
      ...
      <div className="border-t border-white/5 bg-black/20 py-4">
        <div className="mx-auto max-w-7xl px-4 flex flex-col md:flex-row justify-between items-center gap-2">
          <p className="text-[8px] md:text-[12px] text-slate-500 font-medium">© {currentYear} Cracklix</p>
          <div className="flex items-center gap-2 text-[8px] md:text-[11px] font-black uppercase text-slate-600 tracking-tight">
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode; }) {
  return (
    <li>
      <Link href={href} className="text-[10px] md:text-sm text-slate-400 hover:text-white transition-colors font-bold uppercase tracking-tight">
        {children}
      </Link>
    </li>
  );
...
function SocialIcon({ href, icon }: { href: string; icon: React.ReactNode; }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" className="flex h-8 w-8 items-center justify-center rounded-xl bg-white/5 text-white hover:bg-primary transition-all border border-white/5">
      {icon}
    </a>
  );
}