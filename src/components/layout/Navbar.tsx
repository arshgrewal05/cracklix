...
/**
 * @fileOverview High-Fidelity Mobile-First Navbar v195.0.
 * UPDATED: Optimized for PWA with safe-area support and fixed 100px logo fit.
 */
export default function Navbar() {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (prompt) {
        ...
        toast({ 
          ...
          description: "If you don't see a prompt, the app might already be installed." 
        });
      }
    }
  };
  ...
  return (
    <div className="sticky top-0 z-[1000] w-full pointer-events-auto font-body text-left">
      <nav className={cn(
        "w-full flex items-center bg-[#0B1528] border-b border-white/5 px-4 md:px-8 shadow-2xl overflow-visible backdrop-blur-xl",
        headerHeightClass
      )}>
        <div className="container mx-auto max-w-[1536px] flex items-center justify-between h-full gap-2">
          ...
          <div className="flex items-center gap-2 md:gap-4 shrink-0 h-full">
            <button 
              ...
              className="w-9 h-9 bg-white/5 text-white rounded-[10px] border border-white/10 hover:bg-white/10 transition-all flex items-center justify-center cursor-pointer active:scale-90 outline-none"
            >
            ...
            <div className="flex items-center shrink-0 h-full overflow-visible">
               <Logo 
                 imgClassName="h-[100px] origin-left scale-75 md:scale-100" 
          ...
          <div className="hidden lg:flex flex-1 items-center justify-center gap-4 xl:gap-8 h-full">
            ...
            <Link href="/pass" className="transition-all active:scale-95 group h-full flex items-center">
              <div className={cn(
                "px-4 h-[38px] min-w-[120px] rounded-[12px] border flex items-center justify-center gap-2 transition-all shadow-lg",
                pathname === "/pass" 
          ...
          <div className="flex items-center gap-2 md:gap-4 shrink-0 h-full">
             {user && (
                <div 
                  ...
                  className={cn(
                    "hidden sm:flex h-[38px] px-4 min-w-[130px] rounded-xl items-center justify-center gap-2 shadow-xl shrink-0 group border transition-all cursor-pointer active:scale-95",
               ...
               {user ? (
                 <DropdownMenu>
                   <DropdownMenuTrigger asChild>
                     <button className="w-9 h-9 md:w-[42px] md:h-[42px] rounded-full overflow-hidden border-[2.5px] border-white/10 hover:border-primary transition-all bg-white shadow-2xl focus:outline-none flex items-center justify-center active:scale-90 cursor-pointer shrink-0">
                   ...
                   <DropdownMenuContent className="w-64 bg-[#0F172A] border-white/10 text-white rounded-2xl p-2 shadow-5xl z-[2001] mt-2" align="end">
                     ...
                     <DropdownMenuItem asChild className="flex items-center gap-4 px-4 py-3.5 cursor-pointer rounded-xl transition-all focus:bg-white/5 focus:text-white group">
                       <Link href="/my-exams" className="w-full flex items-center gap-4">
                         ...
                         <span className="font-bold text-[13px] tracking-tight uppercase">MY RESULTS</span>
                       </Link>
                     </DropdownMenuItem>
                     {isAdmin && (
                       <DropdownMenuItem asChild className="flex items-center gap-4 px-4 py-3.5 cursor-pointer rounded-xl transition-all bg-white/5 focus:bg-white/10 group mt-1 border border-white/5">
                         <Link href="/admin" className="w-full flex items-center gap-4">
                           ...
                           <span className="font-bold text-[13px] tracking-tight uppercase text-white">ADMIN HUB</span>
                         </Link>
                       </DropdownMenuItem>
                     )}
                     ...
                     <DropdownMenuItem onClick={handleLogout} className="flex items-center gap-4 px-4 py-3.5 cursor-pointer rounded-xl transition-all focus:bg-rose-50/10 group">
                       ...
                       <span className="font-bold text-[13px] tracking-tight uppercase text-rose-500">LOG OUT</span>
                     </DropdownMenuItem>
                   </DropdownMenuContent>
                 </DropdownMenu>
               ) : (
                 <Button asChild className="bg-primary hover:bg-orange-600 text-white font-black px-4 md:px-6 h-9 md:h-11 uppercase text-[10px] md:text-[11px] tracking-widest shadow-xl border-none transition-all active:scale-95">
...
function NavLink({ icon, label1, label2, href, active }: { icon: React.ReactNode, label1: string, label2: string, href: string, active: boolean }) {
  return (
    <Link href={href} className={cn(
      "flex items-center gap-2.5 group transition-all active:scale-95 h-full",
      active ? "opacity-100" : "opacity-60 hover:opacity-100"
    )}>
      <div className={cn(
        "h-8 w-8 rounded-[10px] flex items-center justify-center shadow-md transition-all",
