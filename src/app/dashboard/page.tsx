 ...
 * Features: Readiness Score, Selection Probability, and Subscription Gating.
 */
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <Card className="border-none shadow-3xl shadow-slate-900/5 rounded-[3.5rem] bg-white overflow-hidden group">
               ...
               <CardContent className="pt-16 pb-12 px-10 space-y-6">
                  <div className="text-left">
                    <h2 className="text-3xl font-headline font-black text-[#0F172A] tracking-tight text-left">{profile?.name}</h2>
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mt-1 text-left">{profile?.email}</p>
                  </div>
            ...
            <Card className="border-none bg-white p-10 rounded-[3rem] shadow-3xl space-y-8">
               <div className="flex items-center justify-between">
                  <div className="space-y-1 text-left">
                     <p className="text-[10px] font-black uppercase tracking-widest text-primary text-left">Milestone Hub</p>
                     <h3 className="text-2xl font-headline font-black text-[#0F172A] text-left">Aspirant Badges</h3>
                  </div>
               ...
               <div className="grid grid-cols-3 gap-4">
                  ...
                  <BadgeIcon icon={<Star />} label="Pro Node" active={profile?.status !== 'Free'} />
               </div>
            </Card>

            {/* Pass Status Card */}
            <Card className={`border-none p-10 rounded-[3rem] shadow-3xl relative overflow-hidden text-left ${profile?.status === 'Free' ? 'bg-[#0F172A] text-white' : 'bg-emerald-600 text-white'}`}>
               <div className="absolute top-0 right-0 p-6 opacity-10"><CreditCard className="h-32 w-32" /></div>
               <div className="relative z-10 space-y-6">
                  <div className="flex items-center gap-3">
                     <ShieldCheck className="h-5 w-5" />
                     <span className="text-[10px] font-black uppercase tracking-[0.2em]">Institutional Access Node</span>
                  </div>
                  <div className="space-y-1">
                    <h3 className="text-3xl font-headline font-black leading-tight uppercase">{profile?.status || 'Free'} Pass</h3>
                    <p className="text-xs font-bold uppercase tracking-widest opacity-70">Valid Cycle: 30 Days Audit</p>
                  </div>
                  {profile?.status === 'Free' && (
                    <Button asChild className="w-full bg-primary hover:bg-primary/90 h-14 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl">
                       <Link href="/pricing">Upgrade to Gold Pass</Link>
                    </Button>
                  )}
               </div>
            </Card>
          ...
          <div className="lg:col-span-8 space-y-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
               <div className="space-y-1 text-left">
                  <h1 className="text-4xl font-headline font-black text-[#0F172A] tracking-tight uppercase text-left">Performance Engine</h1>
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-[0.3em] text-left">Institutional Readiness Audit</p>
               </div>
            ...
            {lastSession && (
               <Card className="border-none bg-orange-500 text-white p-10 rounded-[3rem] shadow-3xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8 group">
                  <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:rotate-12 transition-transform text-left"><PlayCircle className="h-32 w-32" /></div>
                  <div className="space-y-3 relative z-10 text-left">
                     <p className="text-[10px] font-black uppercase tracking-[0.3em] text-orange-100 text-left">Interrupt Detected</p>
                     <h3 className="text-3xl font-headline font-black uppercase leading-tight text-left">Resume {lastSession.mockId.split('-')[0]} Audit?</h3>
                     <p className="text-orange-50 text-sm font-bold opacity-80 uppercase tracking-widest text-left">Saved at Node {lastSession.currentIdx + 1} • {Math.floor(lastSession.remainingTime / 60)}m left</p>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
               ...
               <Card className="border-none shadow-3xl shadow-slate-900/5 rounded-[3.5rem] bg-white p-12 text-center space-y-8 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:scale-125 transition-transform text-center"><CheckCircle2 className="h-40 w-40" /></div>
                  <div className="space-y-2 relative z-10 text-center">
                     <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 text-center">Readiness Score</p>
                     <p className="text-8xl font-headline font-black text-[#0F172A] tracking-tighter text-center">{analytics.readinessScore}%</p>
                  ...
                  <div className="space-y-4 pt-4 border-t border-slate-50 relative z-10 text-center">
                     <p className="text-xs font-bold text-slate-500 uppercase tracking-widest text-center">Target: {profile?.targetExam || 'General'}</p>
               ...
               <Card className="border-none shadow-3xl shadow-slate-900/10 rounded-[3rem] bg-[#0F172A] text-white p-10 space-y-8 relative overflow-hidden flex flex-col justify-between text-left">
                  <div className="absolute top-0 right-0 p-6 opacity-10 text-left"><BrainCircuit className="h-32 w-32 text-primary" /></div>
                  <div className="space-y-6 relative z-10 text-left">
                     <div className="flex items-center gap-4 text-left">
                        <div className="h-12 w-12 bg-primary/20 rounded-2xl flex items-center justify-center">
                           <BrainCircuit className="h-6 w-6 text-primary" />
                        </div>
                        <h3 className="font-headline font-black text-xl uppercase tracking-tight text-left">AI Audit Engine</h3>
                     </div>
                     <div className="space-y-4 text-left">
                        ...
                        <div className="p-6 bg-white/5 rounded-2xl border border-white/5 flex items-center justify-between group cursor-pointer hover:bg-white/10 transition-all text-left" onClick={() => router.push('/mocks')}>
                           <div className="space-y-1 text-left">
                              <p className="text-[10px] font-black uppercase text-primary tracking-widest text-left">Recommended Series</p>
                              <p className="font-bold text-slate-200 text-left">{analytics.weakSubject} Mastery</p>
                           </div>
                  ...
                  <div className="pt-8 border-t border-white/5 relative z-10 text-left">
                     <div className="flex justify-between items-end mb-4 text-left">
                        <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest text-left">Selection Probability</p>
                        <p className="text-2xl font-headline font-black text-emerald-500 text-left">{analytics.selectionProb}%</p>
                     </div>
